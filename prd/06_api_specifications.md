...
This section details the comprehensive REST API that powers the Impala application. The API follows RESTful principles and includes all endpoints for core functionality, AI integration, collaboration, and enterprise features.
...
Staging: https://api-staging.impala.ai/v1
...
- **`POST /auth/oauth/{provider}`**
  ...
  - **Request Body:**
    ```json
    {
      ...
      "redirect_uri": "https://app.impala.ai/auth/callback"
    }
    ```
...
- **Connection:** `wss://api.impala.ai/v1/ws/{canvas_id}?token={jwt_token}`
...
This comprehensive API specification provides complete coverage of all Impala features while maintaining consistency with the technical architecture and user flows.
